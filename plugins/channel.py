...
async def _process_with_lock(bot, filename, caption, media_info, base_name, processed):
    ...
    if not movie_doc:
        ...
        movie_doc = {
            "_id": base_name,
            "files": [file_data],
            "poster_url": (details.get("backdrop_url")if (LANDSCAPE_POSTER and TMDB_POSTER and not error_tmdb and details.get("backdrop_url"))else details.get("poster_url")),
            "genres": genres,
        ...
        try:
            ...
            buttons = InlineKeyboardMarkup([[
                InlineKeyboardButton(
                    '♻️Movie Search Group ♻️',
                    url=f"https://t.me/Rk2x_Request"
                )
            ]])

            if movie_doc.get("poster_url") and not LINK_PREVIEW:
                resized_poster = await fetch_image(movie_doc["poster_url"], size=(2560, 1440) if LANDSCAPE_POSTER and TMDB_POSTER and not error_tmdb else (2560, 1440))
